(function() {
    ...
    // keyword filter. let the user debug queries that contain 
    // certain keywords. includes stacktraces so that the origin 
    // may be determined
    let keyWordDebug = false;
    let keyWordRegexp;
    const keywords = process.argv
        .filter(item => item.startsWith('--related-keywords='))
        .map(item => item
            .substr(19)
            .split(/\s*,\s*/)
    ...
    if (keywords.length) {
        keyWordDebug = true;
        keyWordRegexp = new RegExp(`^(?=.*\\b${keywords.join(`\\b)(?=.*\\b`)}\\b).*$`, 'gi');
    }
    ...
    module.exports = new Class({
        ...
        , query: function(queryContext, values) {
            if (this.ended || this.killed) return Promise.reject(new Error('Cannot execute the query, the connection has ended!'));
                ...
                if (keyWordDebug) {
                    // get a string containing all values
                    const testString = queryContext.sql + ', ' + queryContext.values.join(', ');
                    if (keyWordRegexp.test(testString)) {
                        queryContext.debug = true;

                        if (!queryContext.stack) {
                            const len = Error.stackTraceLimit;
                            Error.stackTraceLimit = 10000;
                            queryContext.stack = new Error('sql debugger stack capture').stack;
                ...
                // indicate that a query is being executed
    ...
    ${this.createDebugBanner('SQL DEBUGGER', true).grey}
            ...
            if (queryContext.stack) {
                console.log(`\n\nStack`.yellow.bold);
                console.log(queryContext.stack);
            }
        }
