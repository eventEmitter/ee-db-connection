	...
	module.exports = new Class({
		...
		, query: function(){
			var   mode 			= arg(arguments, 'string', 'query')
				, queries 		= arg(arguments, 'array', [arg(arguments, 'object', null)])
				, callback 		= arg(arguments, 'function', function(){})
				, readOnly 		= arg(arguments, 'boolean', true)
				, parameters 	= arg(arguments, 'object', {})
				, canBleed 		= false
				, SQLString
				, data
				, parameters;
			...
			// chekc if we got an sql query
			if (queries.length === 1 && queries[0] === null && mode !== 'query') {
				queries = [mode];
				mode = 'query';
			}
			...
			async.each(queries, function(query, next) {
				// render query
				if (type.string(query)) next(null, {SQLString: query, parameters: parameters});
				else next(null, this._render(mode, query));
		...
		, _fillByArray: function(SQLString, parameters) {
			...
			while(result = reg.exec(SQLString)){
				if (parameters.length >= index) {
					newString += result.substring(offset, result.index) + (parameters[index -1] === null ? null : this.escape(this._toString(parameters[index -1])));
					index++;
