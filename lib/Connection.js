(function() {
    ...
    module.exports = new Class({
                ...
                if (value !== this._idle) {
                    ...
                    if (this.pooled) {
                        if (value) {
                            ...
                            if (!this.ended && !this.killed) this.emit('idle');
                        }
                        else this.emit('busy');
                    }
                }
        ...
        , query: asyncMethod(function(query, callback) {
            if (this.ended || this.killed) callback(new Error('Cannot execute the query, the connection has ended!'));
                ...
                // execute the query
                this.executeQuery(query, (err, data) => {
        ...
        , createTransaction: asyncMethod(function(callback) {
            if (this.ended || this.killed) return callback(new Error('Cannot start transaction, the connection has ended!'));
        ...
        , commit: asyncMethod(function(callback) {
            ...
            // query
            this.query({SQL: 'commit;', mode: 'transaction'}, (err) => {
        ...
        , rollback: asyncMethod(function(callback) {
            ...
            // query
            this.query({SQL: 'rollback;', mode: 'transaction'}, (err) => {
