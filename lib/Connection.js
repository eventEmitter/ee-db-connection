!function(){
    ...
    module.exports = new Class({
        ...
        , init: function(options, id) {
            ...
            // since connections may die without any notifications we 
            // need to poll our own status every x seconds
            var interval = setInterval(function() {
                if (this._idle) {
                    var timeout = setTimeout(function() {
                        if (this._idle) {
                            // we needd to trmeinate
                            this.end();
                        }
                    }.bind(this), 20000);
                    ...
                    // pinf the db
                    this.queryRaw('SELECT 1;', function(err) {
                        clearTimeout(timeout);
                    }.bind(this));
                }
            }.bind(this), 30000);
        ...
        , _render: function(configuration) {
            ...
            if (!this._validModes[configuration.mode]) {
                if (configuration.callback) configuration.callback(new Error('Invalid query mode «'+configuration.mode+'» !'));
                return false;
            }
            else {
                // render
                ...
                configuration.parameters    = result.parameters.get();
         ...
         */
        , end: function(handlingEnd) {
            return new Promise(function(resolve, reject) {
                if (this._idle || handlingEnd) {
                    this._ended = true;
                    this._setBusy();
                    ...
                    if (this.connection) this.connection.on('end', resolve);
                    else resolve();
                }
                else {
                    this.off('idle');
                    this.once('idle', function() {
                        this.end(true).then(resolve).catch(reject);
                    }.bind(this));
