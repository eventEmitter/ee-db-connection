(function() {
    ...
    module.exports = new Class({
                        ...
                        if (value) {
                            ...
                            if (this.debug) {
                                ...
                                this.idletimer = setTimeout(() => {
                                    log.warn(`Query is still busy after ${timeoutTime}ms`);
                                    if (this.lastQueryContext) {
                                        this.printDebugInfo(this.lastQueryContext);
                                    }
                                }, timeoutTime);
                            }
                        }
        ...
        , init: function(config, id, node) {
            ...
            this.debug = process.argv.includes('--debug-db-connection');
        ...
        , query: function(queryContext, values) {
            if (this.ended || this.killed) return Promise.reject(new Error('Cannot execute the query, the connection has ended!'));
                ...
                if (!queryContext || typeof queryContext.isValid !== 'function') {
                    ...
                    // used for debugging hanging queries
