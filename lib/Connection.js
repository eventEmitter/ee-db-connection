(function() {
    ...
    var   Class         = require('ee-class')
        ...
        , sqlformatter  = require('sqlformatter')
        , relatedSlow   = argv.has('related-slow')
        , relatedSQL    = argv.has('related-sql')
        , relatedErrors = argv.has('related-errors')
        ;
    ...
    module.exports = new Class({
        ...
        , query: function(queryContext, values) {
            if (this.ended || this.killed) return Promise.reject(new Error('Cannot execute the query, the connection has ended!'));
                ...
                if (!queryContext.isValid()) return Promise.reject(new Error(`The query is invalid: ${queryContext.invalidBecauseOf()}`));
                else {//log(queryContext)
    ...
    ${this.createDebugBanner('SQL DEBUGGER', true).grey}
        ...
        , rollback: function() {
            ...
            // query
            return this.query(new QueryContext({sql: 'rollback;', mode: 'transaction'}), (data) => {
